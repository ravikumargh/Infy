...
/**
 * Validate Session Secret parameter is not set to default in categoryion
 */
var validateSessionSecret = function (config, testing) {

  if (process.env.NODE_ENV !== 'categoryion') {
    return true;
  }

  if (config.sessionSecret === 'INFY') {
    if (!testing) {
      console.log(chalk.red('+ WARNING: It is strongly recommended that you change sessionSecret config while running in categoryion!'));
      console.log(chalk.red('  Please add `sessionSecret: process.env.SESSION_SECRET || \'super amazing secret\'` to '));
      console.log(chalk.red('  `config/env/categoryion.js` or `config/env/local.js`'));
      console.log();
    }
    return false;
  } else {
...
var initGlobalConfig = function () {
  ...
  // read package.json for Infy project information
